...
<div class="bg-gray-100 min-h-screen py-10">
    ...
    <form action="{{ route('update.role', $role->id ) }}" method="POST">
        ...
        <input name="name" value="{{ old('name', $role->name) }}" class="w-full border border-gray-300 px-4 py-2 rounded-md text-base focus:outline-none focus:ring-2 focus:ring-blue-400" type="text">
        @error('name')
            <span class=" mt-2 border border-gray-600 bg-red-400 rounded-sm py-2 px-3">{{$message}}</span>
        @enderror
        <div class="mt-2">
            <button class="mt-2 border-black border bg-blue-600 rounded-sm text-white py-2 px-5 hover:bg-blue-700 duration-300">Update</button>
        </div>
    </div>
      <div class="mb-6">
        ...
        @if ($permissions->isNotEmpty())
            @foreach ($permissions as $permission)
                <label for="permission-{{ $permission->id }}" class="flex items-center gap-3 p-3 bg-gray-50 border border-gray-200 rounded-lg hover:shadow-md transition cursor-pointer">
                    <input {{ ($hasPermissions->contains($permission->name)) ? 'checked' : ''}}  type="checkbox" id="permission-{{ $permission->id }}" name="permission[]"
                        value="{{ $permission->id }}"
